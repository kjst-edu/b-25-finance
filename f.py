...
for col in df.columns:
    ...
    elif "貸付金" in col and "都道府県財政" in col:
        x_var_choices[col] = "貸付金（都道府県財政）"

if has_kashidashikin:
    x_var_choices["都道府県別貸出金"] = "都道府県別貸出金"

# Y軸の変数の選択肢を動的に作成（元のデータ + 新しい売上データ）
y_var_choices = x_var_choices.copy()

for col in df.columns:
    if "農林漁業" in col:
        y_var_choices[col] = "売上金額(農林漁業)"
    elif "農業、林業" in col or "農業,林業" in col:
        y_var_choices[col] = "売上金額(農業、林業)"
    elif "漁業" in col and "農林" not in col:
        y_var_choices[col] = "売上金額(漁業)"
    elif "鉱業" in col:
        y_var_choices[col] = "売上金額(鉱業等)"
    elif "建設業" in col:
        y_var_choices[col] = "売上金額(建設業)"
    elif "製造業" in col:
        y_var_choices[col] = "売上金額(製造業)"
    elif "サービス産業" in col or "C6301" in col:
        y_var_choices[col] = "売上高(サービス産業)"

# デフォルトの選択を設定
x_default = list(x_var_choices.keys())[0] if x_var_choices else None
y_default = list(y_var_choices.keys())[0] if y_var_choices else None
